...
The script is very customizable, allowing you to specify
    ...
    * screen or client window coordinates
...
class MainFrame (wx.Frame):
    ...
    def __init__(self):
        ...
        self.previousActiveWindowHwnd = win32gui.GetForegroundWindow()
        ...
        self.origCursorX, self.origCursorY = win32api.GetCursorPos()
        ...
        wx.CallAfter(self.restoreWindow)
    ...
    def CalculateGrid(self):
		...
		#	self.clientWidth
		#	self.clientHeight
		#	self.columnWidth (or self.gridWidth and self.numColumns)
		#	self.rowHeight (or self.gridHeight and self.numRows)
		#	self.gridWidth
		#	self.gridHeight
        ...
        if self.positionMode:
            ...
            self.gridX = self.screenClientX + self.LABEL_WIDTH
            self.gridY = self.screenClientY + self.LABEL_HEIGHT
        else:
            ...
            if self.sizeToClient != None:
                ...
                if self.isInteger(self.sizeToClient):
                    ...
                    try:
                        win32gui.EnumWindows(CustomGridUtilities.checkHwndForProcessName, self.sizeToClient)
                    except:
                        # enumeration complete
                        pass

                    # Found hwnd is stored in mycommon global variable
                    if CustomGridUtilities.foundProcessHwnd == None:
                        CustomGridUtilities.messageBoxOK('Cannot find window for process "' + self.sizeToClient + '".', "Custom Grid Error")
                        self.DoExit()
                    processHwnd = CustomGridUtilities.foundProcessHwnd

                # Are x and y provided?
                #CustomGridUtilities.messageBoxOK('"' + self.gridY + '"', "Debug")
                if self.gridX == 0 or self.gridY == 0:
                    # Use entire client window 
                    self.screenClientX, self.screenClientY = win32gui.ClientToScreen(processHwnd, (0, 0))
                else:
                    # Custom grid relative to client
                    self.screenClientX, self.screenClientY = win32gui.ClientToScreen(processHwnd, (self.gridX, self.gridY))
                    #self.screenClientX = self.screenClientX - self.LABEL_WIDTH
                    #self.screenClientY = self.screenClientY - self.LABEL_HEIGHT
                                  
                # Are grid width and height provided?
                if self.gridWidth == None or self.gridHeight == None:
                    # Use window client size
                    # returns a tuple (left, top, right, bottom)
                    clientrect = win32gui.GetClientRect(processHwnd)

                    # Position the grid INSIDE the client area so that it shows when window is maximized
                    self.clientWidth = clientrect[2] - clientrect[0]
                    self.clientHeight = clientrect[3] - clientrect[1]
                    self.gridWidth = self.clientWidth
                    self.gridHeight = self.clientHeight
                else:
                    # Custom size relative to client
                    #self.gridWidth = self.gridWidth + self.LABEL_WIDTH * 2
                    #self.gridHeight = self.gridHeight + self.LABEL_HEIGHT * 2
                    self.clientWidth = self.gridWidth
                    self.clientHeight = self.gridHeight
                ...
                self.screenClientX = self.gridX - self.LABEL_WIDTH
                self.screenClientY = self.gridY - self.LABEL_HEIGHT

            # Client area and window area are the same because of no border
            self.SetRect(wx.Rect(self.screenClientX , self.screenClientY, self.gridWidth, self.gridHeight))
    ...
    def OnKeyDown (self, event) :
        ...
        if keycode == wx.WXK_ESCAPE:
            ...
            if keycode >= ord('0') and keycode <= ord('9'):
                ...
                self.doHide()
                self.FinishKeyMode()
                Timer(0.1, self.dragClickMouse).start()
            elif key == 'y':
                ...
                CustomGridUtilities.messageBoxOK(helpString, "Custom Grid Help")
            else:
                self.FinishKeyMode()
    ...
    def dragClickMouse(self):
        # Move back to original location and press down
        win32api.SetCursorPos((self.origCursorX, self.origCursorY))
        win32api.mouse_event(win32con.MOUSEEVENTF_LEFTDOWN, self.origCursorX, self.origCursorY, 0, 0)
        
        # Drag to new location and release
        win32api.SetCursorPos((self.nextCursorX, self.nextCursorY))
        Timer(1.0, self.dragClickMouseFinish).start()

    def dragClickMouseFinish(self):       
        # Drag to new location and release
    ...
    def stickyShow(self):
        if self.isSticky:
            Timer(self.stickyRefreshDelay, self.delayedShow).start()
    ...
    def ReportValues(self, promptBeforeCopy):
        ...
        useClient = False
        if promptBeforeCopy:
            message = 'Copy the parameters into the clipboard as the commandline switches needed to run the grid?'
            if CustomGridUtilities.messageBoxYesNo(message, "Custom Grid Results") == CustomGridUtilities.MESSAGEBOX_RETURN_YES:
                doCopy = True
                message = "Use screen coordinates (Yes) or client coordinates (No)"
                if CustomGridUtilities.messageBoxYesNo(message, "Custom Grid Results") == CustomGridUtilities.MESSAGEBOX_RETURN_NO:
                    useClient = True
        else:
            doCopy = True
        ...
        if useClient:
            # Use client coordinates for location x and y
            
            newGridX, newGridY = win32gui.ScreenToClient(self.previousActiveWindowHwnd, (self.gridX, self.gridY))
            self.gridX = newGridX
            self.gridY = newGridY
            # CustomGridUtilities.messageBoxOK(win32gui.GetWindowText(self.previousActiveWindowHwnd), "Test")

        data = "Grid location x: " + str(self.gridX)  + \
        ...
        if doCopy:                
            CustomGridUtilities.setClipboard('--width ' + str(self.gridWidth) + \
                                   ' --height ' + str(self.gridHeight) + \
                                   ' --locationx ' + str(self.gridX) + \
                                   ' --locationy ' + str(self.gridY) + \
            ...
            CustomGridUtilities.messageBoxOK(data, "Custom Grid Results")
...
'''Once the grid is displayed, you can control the application using the following commands:
    ...
    def ProcessCommandline(self, argsString=None):
        ...
        try:
            ...
            CustomGridUtilities.messageBoxOK("Error while parsing arguments: " + ex.message, 'Custom Grid Error')

        self.positionMode = args.positionMode
        self.gridWidth = args.width
        self.gridHeight = args.height
        self.gridX = args.locationx
        self.gridY = args.locationy
        ...
        if self.sizeToClient != None:
            if self.sizeToClient == '':
                self.DoExit('The value for the sizeToClient must be an application file name or HWND value.')
